...
static void genCoil (float pitch) {
	...
	float yshift = -(N-1)*0.4/2;
	int i;
	

	for (i=0; i<N; i++){

		x[2*i][0] = R_INTER/2* sin(stepAngle+M_PI);
		x[2*i][1] = yshift + i* 0.4;
		x[2*i][2] = R_INTER/2* cos(stepAngle+M_PI) ;
		
		x[2*i+1][0] = R_INTER/2 *sin(stepAngle) ;
		x[2*i+1][1] = yshift + i* 0.4;
		x[2*i+1][2] = R_INTER/2 *cos(stepAngle);
...
// generate a ladder hairpin

// generate a twisted hairpin
